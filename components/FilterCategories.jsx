...
function FilterCategories({
  ...
  return (
    <>
      <div className='flex text-sm sm:text-base md:text-md lg:text-lg max-w-sm sm:max-w-2xl justify-center items-center'>
        {categories.map((categoryObj) => (
          <div
            key={categoryObj.key}
            className='p-2 text-center flex items-center justify-center flex-col'
          >
            <button
              className={`border-2 rounded lg:h-12 lg:w-12 md:w-10 md:h-10 sm:w-8 sm:h-8 w-6 h-6 flex items-center justify-center ${
                selectedCategory === categoryObj.category ? "bg-gray-200" : ""
              }`}
              onClick={() => filterHandle(categoryObj.category)}
            >
              {categoryObj.icon}
            </button>
            <span>{t(categoryObj.label)}</span>
          </div>
        ))}
      </div>
    </>
