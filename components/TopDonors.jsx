...
function TopDonors({ data }) {
  ...
  const donorList = Object.entries(donorTotals).map(([uid, totalDonation]) => {
  ...
  const getUsersData = async (topDonators) => {
  ...
  const fetchData = async () => {
    const topDonators = donorList.slice(0, 3);
    const userData = await getUsersData(topDonators);
    ...
    <div className='flex flex-col'>
      ...
      <div>
        <Image
          ...
          className='rounded'
          alt='user image'
          src={donators[0]?.photoURL}
        />
        {donators[0]?.displayName}
        <p>{donators[0]?.totalDonation} $</p>
      </div>
    </div>
  );
}
